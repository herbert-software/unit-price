...
/**
 * Read the LLM config from the environment. Throws `ConfigError` if the API key
 * is missing/empty. The model is taken from the configuration constant unless
 * overridden (kept for testability / the future upgrade tier).
 */
export function loadLlmConfig(env: NodeJS.ProcessEnv = process.env, model = DEFAULT_MODEL): LlmConfig {
...
/** Startup fail-fast helper: returns true if config is present. */
export function configPresent(env: NodeJS.ProcessEnv = process.env): boolean {
  const apiKey = env[API_KEY_ENV];
